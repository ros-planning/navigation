...
main(int argc, char** argv)
{
    ...
    amcl_node_ptr = boost::make_shared<AmclNode>();
...
#endif
{
    ...
    ROS_INFO("%s","Using AMCL augmented by Alice Gong (alice.xinyig@gmail.com)");
    ...
    private_nh_.param("update_min_a", a_thresh_, M_PI/10.0);//M_PI/6.0);
    private_nh_.param("odom_frame_id", odom_frame_id_, std::string("odom")); // FIXME
    ROS_INFO("odom_frame_id: %s\n", odom_frame_id_.c_str());
    private_nh_.param("base_frame_id", base_frame_id_, std::string("base_link"));
    ROS_INFO("base_frame_id: %s\n", base_frame_id_.c_str());
...
#ifdef BUILD_DEBUG
    if (debugger) throw std::logic_error("Can only debug 1 AMCL node at a time!");
    debugger = boost::make_shared<AmclDebug>(&nh_, &pf_);
    debugger->importParams(global_frame_id_, base_frame_id_, save_pose_period);
#endif
    ...
    odom_sub_ = nh_.subscribe("odom", 1000, &AmclNode::odomReceived, this);
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
    ...
    pf_init_pose_cov.m[0][0] = last_published_pose.pose.covariance[0];
    pf_init_pose_cov.m[1][1] = last_published_pose.pose.covariance[7];
    pf_init_pose_cov.m[2][2] = last_published_pose.pose.covariance[35];
    ...
    // Odometry
    delete odom_;
    odom_ = new AMCLOdom(a_thresh_, d_thresh_);
    ROS_ASSERT(odom_);
...
AmclNode::checkLaserReceived(const ros::TimerEvent& event)
{
    ...
    if (d <= laser_check_interval_) return;
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
...
#ifdef BUILD_DEBUG
    ROS_INFO("Creating particle filter...");
#endif
    ...
    // Odometry
    delete odom_;
    odom_ = new AMCLOdom(a_thresh_, d_thresh_);//new AMCLOdom();
    ROS_ASSERT(odom_);
    odom_->SetModel( odom_model_type_, alpha1_, alpha2_, alpha3_, alpha4_, alpha5_ );
    // Laser
    delete laser_;
...
#ifdef BUILD_DEBUG
    debugger->publish_likelihood_field(map_);
#endif
}
...
// Ensure times of 90 degrees rotation is within twice to either side
int mod90X(int x90)
{
    if (x90 <= -3) return mod90X(x90+4);
    else if (x90 > 3) return mod90X(x90-4);
    return x90;
}

// Calculate how many times of 90 degrees rotation this yaw corresponds too (integer only)
int calculateRotate90X(float yaw)
{
    if (yaw >= 1.57) return 1+calculateRotate90X(yaw - M_PI_2);
    else if (yaw <= -1.57) return -1+calculateRotate90X(yaw + M_PI_2);
    assert (abs(yaw) <= 1e-6);
    return 0;
}

// Find index on internal LikelihoodField grid for each cell from OccupancyGrid message, by inverse of rotation if applicable
int rotateIndex(int rotate90x, int i, int j, size_t leni, size_t lenj)
{
    ROS_ASSERT(i >= 0 && j >= 0);
    ROS_ASSERT(i < leni && j < lenj);
    if (! rotate90x) return i + j * leni;
    else if (rotate90x < 0) return rotateIndex(rotate90x + 1, j, (leni - 1) - i, lenj, leni);
    else return rotateIndex(rotate90x - 1, (lenj - 1) - j, i, lenj, leni);
}

// Calculate specs of LikelihoodField grid accounting for potential rotations
int AmclNode::setMapSpecs( const nav_msgs::OccupancyGrid& map_msg, map_t* map )
{
    ...
    double offset_x = (HALF(map->size_x) - 0.5) * map->scale;
    double offset_y = (HALF(map->size_y) - 0.5) * map->scale;
    // double offset_x = (float(map->size_x) / 2.0 - 0.5) * map->scale;
    // double offset_y = (float(map->size_y) / 2.0 - 0.5) * map->scale;
    double posn_x = map_msg.info.origin.position.x;
    double posn_y = map_msg.info.origin.position.y;

    geometry_msgs::Quaternion q = map_msg.info.origin.orientation;
    tf::Quaternion quat(q.x, q.y, q.z, q.w);
    tf::Matrix3x3 m(quat);
    double roll, pitch, yaw;
    m.getRPY(roll, pitch, yaw);
    ROS_ASSERT(! roll && ! pitch);

    ROS_INFO("Received a %d X %d map @ %.3f m/pix oriented %.3f\n",
    map_msg.info.width,
    map_msg.info.height,
    ...
    if (abs(yaw) >= 1e-6)
    {
        rotate90x = mod90X(calculateRotate90X(yaw));
        ROS_INFO("Rotated map by  %d x 90 degrees\n",
        rotate90x);
        if (rotate90x != 0)
        {
            if (rotate90x % 2 == 0)
            ...
            {
                map->size_x = map_msg.info.height;
                map->size_y = map_msg.info.width;
                // double temp = posn_y;
                double temp = offset_x;
                // offset_x = rotate90x < 0 ? 0 : -offset_y*2;
                // offset_y = rotate90x < 0 ? -temp*2 : 0;
                offset_x = offset_y;
                offset_y = temp;
                posn_y -= rotate90x > 0 ? 0 : offset_y*2.0;
                posn_x -= rotate90x < 0 ? 0 : offset_x*2.0;

            }
        }
    }

    map->origin_x = posn_x + offset_x;
    map->origin_y = posn_y + offset_y;

    ROS_ASSERT(MAP_GXWX(map, posn_x) == 0);
    ROS_ASSERT(MAP_GYWY(map, posn_y) == 0);
    ROS_INFO("Center of map is %.3f, %.3f", map->origin_x, map->origin_y);
#ifdef BUILD_DEBUG
    int gx = MAP_GXWX(map, map->origin_x);
    int hx = floor(HALF(map->size_x));
    ROS_ASSERT(abs(gx - hx) <= 1e-6);
    int gy = MAP_GYWY(map, map->origin_y);
    int hy = floor(HALF(map->size_y));
    ROS_ASSERT(abs(gy - hy) <= 1e-6);
#endif
...
{
    int map_elem = map_msg.info.height * map_msg.info.width;

#ifdef BUILD_DEBUG
    cv::Mat msg = cv::Mat::zeros(cv::Size(map_msg.info.width, map_msg.info.height), CV_8UC1);
    for(int i = 0; i < map_elem; ++i)
    {
        msg.data[i] = map_msg.data[i];
    }
    cv::imwrite("map_msg_debug.bmp", msg);
#endif

    map_t* map = map_alloc();
    ROS_ASSERT(map);

    int rotate90x = setMapSpecs(map_msg, map);

#ifdef BUILD_DEBUG
    ROS_INFO("Copying occupancy state into likelihood field...");
#endif
...
#ifdef BUILD_DEBUG
    AmclDebug::outputMap(map, "map_imported_debug.bmp");
    debugger->publish_likelihood_field(map);
#endif
...
const ros::Time& t, const std::string& f)
{
    ...
    try
    {
        this->tf_->waitForTransform(odom_frame_id_, f, t, this->transform_tolerance_);
...
AmclNode::uniformPoseGenerator(void* arg)
{
    ...
    } while ( MAP_VALID(map,i,j) && (map->cells[MAP_INDEX(map,i,j)].occ_state != -1) )
#endif
    return p;
}
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
...
#ifdef BUILD_DEBUG
    //DEBUG_INFO("Received scan stamped %lu at t = %lu\n", laser_scan->header.stamp.toNSec(), last_laser_received_ts_.toNSec());
#endif
    ...
    if(update)
    {
        ...
        odata.delta = delta;
#ifndef BUILD_DEBUG
        if (delta.v[0] > d_thresh_*2.0)
            DEBUG_INFO("(pf_init && lasers_update) delta = %.3f , %.3f, %.3f\n", delta.v[0], delta.v[1], delta.v[2]);
#endif
    ...
    // If the robot has moved, update filter
    if (lasers_update_[laser_index])
    {
        ...
        ldata.ranges.resize(ldata.range_count);

#ifdef BUILD_DEBUG
        ldata.pz = std::vector<double>(ldata.range_count, NAN);
#endif
        ...
        if (!m_force_update)
        {
            publishParticleCloud(laser_scan->header.stamp, set);
        }
...
#ifdef BUILD_DEBUG
            debugger->publish_cluster(hyp_count, weight, &pose_mean, &pose_cov);
#endif
        }
        assert(max_weight_hyp >= 0);
        ...
        else
        {
...
#ifdef BUILD_DEBUG
            debugger->publish_clusters_cloud();
#endif
            ...
            for(int i=0; i<2; i++)
            {
                for(int j=0; j<2; j++)
                {
                    ...
                    if (p.pose.covariance[6*i+j] == 0.0) zero_cov = true;
                }
            }
            // Report the overall filter covariance, rather than the
            // covariance for the highest-weight cluster
            ...
            if (p.pose.covariance[6*5+5] == 0.0) zero_cov = true;
            pf_cluster_t* heavy_c = (set->clusters+max_weight_hyp);
            if (heavy_c->count <= (set->sample_count / 10))
            {
                ROS_WARN("[PARTICLES TOO SPARSE] Heaviest cluster # %d / %d, has ONLY %d particles, weighs %lf, scores %lf",
                    max_weight_hyp, set->cluster_count, heavy_c->count, heavy_c->weight, heavy_c->score);
            }
            else if (zero_cov)
            {
                ROS_WARN("[0 in Covariance] Heaviest cluster # %d / %d, has %d particles, weighs %lf, scores %lf",
                    max_weight_hyp, set->cluster_count, heavy_c->count, heavy_c->weight, heavy_c->score);
            }
...
#ifdef BUILD_DEBUG
            debugger->publish_pose(p);
            debugger->publish_scan_at_pose(*laser_scan, scan_pz);
#endif
    ...
    publishParticleCloud(laser_scan->header.stamp, set);
#ifdef BUILD_DEBUG
    ROS_INFO("After resampling, num samples: %d\n", set->sample_count);
    DEBUG_INFO("}--------------------------------------------------------------------------\n");
#endif
...
void
AmclNode::publishParticleCloud(const ros::Time& stamp, pf_sample_set_t* set)
{
...
AmclNode::applyInitialPose()
{
    ...
    if( initial_pose_hyp_ != NULL && map_ != NULL ) {
        ...
        pf_init_ = false;
        ROS_INFO("================ Applied initial pose: %.3f %.3f yaw %.3f ================\n",
