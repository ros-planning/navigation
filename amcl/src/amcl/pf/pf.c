...
{
  srand48(time(NULL)); // QUESTION: what is this for??? can we remove it?

  pf_t* pf = calloc(1, sizeof(pf_t));
...
void pf_update_resample(pf_t* pf)
{
  ...
  */
  ...
  if (w_diff > 0.0) pf->w_slow = pf->w_fast = 0.0;
...
void pf_cluster_stats(pf_t* pf, pf_sample_set_t* set)
{
  ...
  for (int i = 0; i < set->cluster_count; i++)
  {
    ...
    // (I think this is the correct formula for circular statistics.)
                         ...
                         pf_vector_t* mean, pf_matrix_t* cov)
{
  ...
  pf_cluster_t* cluster = set->clusters + clabel;
