...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
    ...
    self = (AMCLLaser*) data->sensor;
    double z_hit_denom = 2.0 * pow(self->sigma_hit, 2);
    double z_rand_pz = self->z_rand / data->range_max;
    double max_occ_dist_prob = exp (- self->map->max_occ_dist / z_hit_denom);

    if (self->max_beams < 1) step = 1;
    else step = (data->range_count - 1) / (self->max_beams - 1);

    // Step size must be at least 1
    if (step < 1) step = 1;
    else if (step >= data->range_count) step = data->range_count;

    double expected_count = std::min(ceil((double)data->range_count/(double)step), (double)data->range_count);
    assert(expected_count >= 1);
    double half_occ_dist = self->map->max_occ_dist/2.0;
    ...
    for (j = 0; j < set->sample_count; ++j)
    {
        ...
        p = 0.0;
        size_t stepped_count = 0;

#ifdef BUILD_DEBUG
        std::vector<double> rays_pz(data->range_count, 0.0);
#endif
        ...
        for (i = 0; i < data->range_count; i += step)
        {
            ...
            hit.v[0] = obs_range * cos(pose.v[2] + obs_bearing);
            hit.v[0] += pose.v[0];
            hit.v[1] = obs_range * sin(pose.v[2] + obs_bearing);
            hit.v[1] += pose.v[1];
            ...
            // Off-map point should not affect (if completely off-map, all particles will weight equal)
            if(!MAP_VALID(self->map, mi, mj)) z = 0;
            else
            {
                z = self->map->cells[MAP_INDEX(self->map,mi,mj)].occ_dist;
                int ambiguous_cell = ! (self->map->cells[MAP_INDEX(self->map,mi,mj)].occ_state);
                if (z >= half_occ_dist && ambiguous_cell) z = 0; // otherwise would have heavily penalize when there is actually no info on map
                else ++stepped_count;
            }
        ...
        assert( p >= 0.0 );
        if (stepped_count > expected_count)
        {
            printf("stepped_count = %lu while expected_count = %lf with ray count = %d and step = %d\n", stepped_count, expected_count, data->range_count, step);
            assert(false);
        }

        double score;

        if (! stepped_count) score = max_occ_dist_prob;
        else
        {
            score = exp ( - p / z_hit_denom / (double)stepped_count);
            // Check if robot is outside of map!
            int mi, mj;
            mi = MAP_GXWX(self->map, pose.v[0]);
            mj = MAP_GXWX(self->map, pose.v[1]);
            int occ_state = self->map->cells[MAP_INDEX(self->map,mi,mj)].occ_state;
            if (occ_state == 1)
            {
                printf("Particle %d: %lf, %lf, yaw %lf in OCCUPIED region!\n", j, pose.v[0], pose.v[1], pose.v[2]);
                score = max_occ_dist_prob;
            }
            // else if (occ_state == 0)
            // {
            //     printf("Particle %d: %lf, %lf, yaw %lf in UNKNOWN region!\n", j, pose.v[0], pose.v[1], pose.v[2]);
            // }
            else if (!MAP_VALID(self->map, mi, mj))
            {
                printf("Particle %d: %lf, %lf, yaw %lf OUT OF MAP!\n", j, pose.v[0], pose.v[1], pose.v[2]);
            }
        }

        sample->score = score * ((double)stepped_count / expected_count); // averaged, then using Gaussian
...
#ifdef BUILD_DEBUG
    double confidence = pow(10.0, log(max_p)/(double)expected_count); // FIXME
    printf("-- max p %.10Le -> confidence = %.10lf\n", max_p, confidence);
    printf("-- total = %.10Le\n", total_weight);
    if (! (total_weight > 0.0))
    return total_weight;
    long double scale = 1.0/total_weight;
    double max_prob = 0.0;
    double min_prob = 1.0;
    for (i = 0; i < data->range_count; i += step)
    {
        data->pz[i] *= scale;
        data->pz[i] = self->z_hit * exp(- pow(data->pz[i], 2) / z_hit_denom) + z_rand_pz;
        if (data->pz[i] > max_prob) max_prob = data->pz[i];
        else if (data->pz[i] < min_prob) min_prob = data->pz[i];
    }
    printf("\t-- Measurement prob: max = %f min = %f\n", max_prob, min_prob);
#endif
...
void AMCLLaser::reallocTempData(int new_max_samples, int new_max_obs){
    ...
    temp_obs.resize(max_samples);
    for(int k=0; k < max_samples; k++){
        temp_obs[k].resize(max_obs);
    }
}
